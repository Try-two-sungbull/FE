    ...
    <Card variant="elevated" className="h-full animate-slide-up" style={{ animationDelay: "0.1s" }}>
      <CardHeader className="flex flex-row items-center justify-between pb-3 border-b">
        ...
        <div className="flex items-center gap-1">
          {hasData && (
            <Button variant="ghost" size="icon-sm" onClick={handleReset} title="원본으로 복원">
              <RotateCcw className="h-4 w-4" />
            </Button>
          )}
          <Button variant="ghost" size="icon-sm" disabled={!hasData} onClick={handleCopy}>
            <Copy className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="icon-sm" disabled={!hasData} onClick={handlePrint}>
        ...
        {hasData && (
          <div className="px-4 py-2 bg-amber-50 border-b flex items-center gap-2">
            ...
            <p className="text-xs text-amber-700">
              <span className="font-medium">문서 전체를 자유롭게 편집</span>할 수 있습니다. 새로운 내용을 추가하거나 기존 내용을 수정하세요.
            </p>
          </div>
        )}
